...
describe('share settings', () => {
  it('starts sharp and slow, and a change touches only the named knob', () => {
    expect(DEFAULT_SHARE).toMatchObject({ frameRate: 30, maxHeight: 0, degradation: 'maintain-resolution', budgetBps: 20_000_000, ceilingBps: 6_000_000 });
    expect(withChange(DEFAULT_SHARE, { frameRate: 60 })).toEqual({ ...DEFAULT_SHARE, frameRate: 60 });
  });
  it('derives track constraints, content hint, and per-viewer encodings', () => {
    const motion = withChange(DEFAULT_SHARE, { frameRate: 60, maxHeight: 720, degradation: 'maintain-framerate' });
    ...
    expect(shareEncoding(DEFAULT_SHARE, 4, 1)).toEqual({ maxBitrate: 5_000_000, maxFramerate: 30, scaleResolutionDownBy: 1, degradationPreference: 'maintain-resolution' });
  ...
  it('parses stored settings defensively', () => {
    ...
    expect(parseShareSettings('{"preset":"motion","frameRate":60}')).toEqual({ ...DEFAULT_SHARE, frameRate: 60 });
  });
