...
- Media is never decrypted by a server. A TURN relay forwarding encrypted bytes is acceptable. The signaling server cannot insert itself either: each description's DTLS fingerprints are signed by the sender's identity key (ADR 0004, 2026-09-10).
...
- **Descriptions are signed** (ADR 0004, 2026-09-10): the sender signs its description's `a=fingerprint` lines, bound to its own and the recipient's public key, with its identity key; the signature travels as `sig` beside the description and the server relays it untouched. The receiver verifies against the public key the server attributed the message to before applying it and drops it otherwise (`signal_rejected`). Candidates are not signed. This turns "encrypted between browsers" into "encrypted between these two identities, whatever the server does to the handshake".
...
- **Attribution**: the server tags every relayed message and presence entry with the sender's verified public key. Clients trust the tag for text and presence. No per-message signatures on text. This trust in our own server is deliberate and recorded in ADR 0003. Signaling descriptions are the exception: they carry an identity signature over the DTLS fingerprints (ADR 0004), so the server cannot place itself inside a call.
- **Names**: self-declared display name plus a six-character fingerprint derived from the public key hash. Duplicates allowed; the UI warns "someone else in the room is also called X" and shows your own fingerprint. Each client keeps a local address book (key, last name, acknowledged-at, optional nickname) and shows a "new" badge on a never-seen key until the user has clicked it once. The fingerprint accompanies a name only while the key is unacknowledged or while two keys show the same name (issue #7); otherwise it sits in the hover title together with "known since". A friend can be given a local nickname (issue #6) that replaces their name in presence, call list, share tiles and chat lines, looked up by key at render time so history lines follow it; the hover title keeps their self-declared name. Clicking an avatar opens the profile card, a popover with the fingerprint, "known since" and the nickname edit; your own card changes the self-declared name for everyone through a `name` message, which the server validates like the auth name and publishes through presence. Texts already sent keep the name they were sent with on the wire; the client shows the current name for anyone present.
...
- **One tab at a time, decided in the browser** (2026-09-24, ticket 25): before opening any socket a tab takes the Web Lock `dave-tab`. A tab that cannot get it within 2 s shows "Dave is already open in another tab", opens nothing, and leaves the first tab working, call included; its request stays queued, so it takes over by itself when that tab closes, and such a tab never Rejoins (§8.1a). "Use it here instead" steals the lock: the tab it was taken from leaves its call while its socket is still open, then waits in turn. A reload of the tab holding the lock steals it back (a sessionStorage flag plus navigation type `reload`), so a waiting tab cannot inherit the app from a refresh. Rooms, the selected room and the name written by the waiting tab (an invite link opened there) reach the running tab through `storage` events. The server's 4004 rule above stays as the backstop for browsers without Web Locks and for races.
- **Presence lists one entry per identity** (2026-09-10, ticket 17): the snapshot takes each public key's newest socket. A superseded or silent socket is marked closing before `close()` and is invisible to presence, fan-out and delivery from then on, because a socket whose peer is already gone can stay attached long after `close()`; with it merely left out of the one broadcast at supersede time, every later broadcast listed the friend twice.
...
- Publishing works wherever `getDisplayMedia` exists: desktop browsers, and iOS Safari from 27 (amended 2026-09-24: a friend on iOS 27 shared on the first try, while friends on Safari 26.6 saw the hint). It does not exist on Android Chrome, Android Firefox, or Safari before 27. Hide the button where it is undefined; the check is on the API, never on the device, so a browser that gains it gets the button with no change here.
...
### 6.4 Bandwidth rule
The sharer's upload is the bottleneck: one encode per watching peer, 6 Mbps per stream by default. Default upload budget 20 Mbps per share, divided equally among active viewers, per-viewer ceiling 6 Mbps, floor 1 Mbps (amended 2026-09-23 from 8 and 2.5 Mbps: the friends using this have the upstream for it), applied as `maxBitrate` on each viewer's connection as subscriptions change. Budget and ceiling are user-configurable. A viewer on a small screen requests half resolution, applied for that peer only with `scaleResolutionDownBy`. Relayed pairs push the same bytes through TURN: about 1.1 GB per hour per 2.5 Mbps stream.
...
- Mobile viewing: `<video playsinline muted>` fed by the received track; voice on a separate audio element with `play()` awaited and a play button on `NotAllowedError`; take a Screen Wake Lock while watching.
- Starting the picture: the tile's video element has no `autoplay` attribute; the app calls `play()` in the click that starts watching, again when the tile turns live or the own capture arrives, and after a source reset. A refused `play()` (Brave with autoplay set to Block, Firefox with the strictest autoplay policy) is logged for problem reports and shows "Click to play" over the tile; that click starts it. Reason: Brave's autoplay block ignores the attribute even for a muted MediaStream and clears the page's user activation whenever the autoplay policy is consulted, which a paused `autoplay` video, an `AudioContext.resume()` or any `play()` does, so nothing that runs before the tile's `play()` in the same gesture may touch media (2026-09-10, ticket 16).
- Black-tile check (ticket 12): 4 s after a tile turns live it must have presented a frame (counted with `requestVideoFrameCallback`; `getVideoPlaybackQuality()` reads zero for a MediaStream in Firefox), have a size, and not be paused; otherwise `share_black` is filed with the element and peer snapshot, the source is re-attached and played, and the share is re-subscribed. Two rounds per subscription.
...
- Sidebar: **Online** (visitors) first, then **Call** (participants). The user is listed last in Online and first in Call, so joining moves their own entry one slot. Entries show avatar initial, name (nickname when set), the fingerprint when the key is new or the name ambiguous, "new" badge, an avatar that opens the profile card, and for participants the muted and sharing flags plus the connection badge.
...
The room is one CSS grid: the sidebar spans every row on the left; shares, chat log and composer are three rows of the right column, the composer being its own grid item. Rare actions ("Report a problem", "Clear chat history", the latter behind a confirm) sit in a footer pinned to the bottom of the sidebar, far from anything clicked often. On phones (one column) the app is exactly one screen tall: the chat row takes what is left below the lists and shares, so the newest lines sit right above the composer without scrolling and older lines scroll inside the log; only when the lists above are taller than the screen does the part above the composer scroll. The composer is the app's last grid row, in flow, so it can never cover a line (safe-area aware), and the viewport declares `interactive-widget=resizes-content` so an open keyboard shrinks the layout instead of covering it. Server-state notices float as a pill from the top edge of the main column and never move the layout.
...
A page that reloads while in the Call joins it again without a click, whatever caused the reload. While in the Call the client keeps a marker in localStorage fresh: `{ room, at, watching }`, written on join, every 10 s, on every watch or unwatch, and on `pagehide` (the heartbeat covers Android killing a background app without any page event; sessionStorage is not used because Chrome for Android does not restore it after a kill). On load, a marker for this room younger than 30 s triggers the Rejoin once the socket is connected. A deliberate Leave removes the marker, and so does another tab of this identity taking over, so the superseded tab needs a click.

The Rejoin is the ordinary join minus the click: microphone first, because a live capture is what lets Chromium and Firefox start remote audio and the AudioContext without a gesture (research `rejoin-without-a-click`), then the join is declared. Mute is the persisted one. **Watched shares come back**: the sharers in the marker that presence still shows sharing are watched again with no click. My own share does not survive: `getDisplayMedia` needs a gesture, so Share screen is simply ready. If the browser still refuses audio, the call controls offer "Click to hear the call". Others see a short drop within the §8.1 grace period and may hear the leave and join cues.

A viewer re-sends its subscription each time a connection to the sharer reaches connected, not only when the connection is built: after a reload or a rebuild the first ask can reach the sharer while they still hold the old connection, which the fresh offer then closes along with the subscription.

### 8.2 Peer connections
Per connection, read `getStats` every 2 s. The selected candidate pair's type gives direct versus relayed. The badge turns direct or relayed only when the full connection state is connected, which includes the DTLS handshake (ticket 14, 2026-09-10: ICE alone can succeed one-sidedly when the other side refused our description, and a path without encryption carries no media); until then the row stays "connecting" and the stuck-connecting watchdog applies. ICE state drives recovery: disconnected is "reconnecting" and triggers an ICE restart after 5 s; failed is "unreachable" and retries ICE restart with backoff. **Server presence wins**: a failed peer link never removes anyone from the Call; only an explicit leave or a dead socket does.
