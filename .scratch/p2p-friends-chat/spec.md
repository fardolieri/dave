...
The room is one CSS grid: the sidebar spans every row on the left; shares, chat log and composer are three rows of the right column, the composer being its own grid item. Rare actions ("Report a problem", "Clear chat history", the latter behind a confirm) sit in a footer pinned to the bottom of the sidebar, far from anything clicked often. On phones (one column) the app is exactly one screen tall: the chat row takes what is left below the lists and shares, so the newest lines sit right above the composer without scrolling and older lines scroll inside the log; only when the lists above are taller than the screen does the whole app scroll. The composer is pinned to the bottom of the screen (`position: fixed`, safe-area aware) and the viewport declares `interactive-widget=resizes-content` so an open keyboard shrinks the layout instead of covering it. Server-state notices float as a pill from the top edge of the main column and never move the layout.
