...
4. **Transceiver model**: every peer connection has fixed transceivers, one voice audio, one share video, one share audio. The initiator pre-adds them; the answerer adopts the three that `setRemoteDescription` creates, sets them to `sendrecv`, and attaches its tracks before answering (spike finding 2026-09-06: JSEP only associates `addTrack`-created transceivers, pre-adding on both sides forces a second round). Deactivating a share for a newly joined peer waits until the answer is applied, since the sender has no encodings before that. Start share is `replaceTrack(track)`, stop is `replaceTrack(null)`. Offer/answer happens only at join and leave. **One share per participant at a time.** UI renders share tiles from signaling state, not `track` events.
