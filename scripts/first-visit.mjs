...
// usage: node scripts/first-visit.mjs <url> <secret> <name>   (the link is <url>#<secret>/First%20Visit)
...
try {
  await cdp('Page.navigate', { url: `${url}#${encodeURIComponent(secret)}/${encodeURIComponent('First Visit')}` }); await sleep(1500);
